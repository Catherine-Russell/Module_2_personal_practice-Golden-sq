class To_do_tracker():
    ...
    def lst_tasks(self):
        ...
        return self._task_lst

    def remove_completed_task(self, task):
        #remove string from task list
        #input task type str
        #output none
        if task in self._task_lst:
            self._task_lst.remove(task)
        else:
            return f"This task is not in your To Do List. Please choose from: {self._task_lst}"